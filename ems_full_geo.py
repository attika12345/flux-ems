...
config = {
    ...
    # PV configuration
    "has_pv":                  True,   # Does the system have solar panels?
    "panel_kwp":               1.76,   # Total PV capacity in kWp
}
...
def load_config():
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            with config_lock:
                config.update(data)
                PANEL_KWP = config.get("panel_kwp", 1.76)  # Update global
...
def compute_cheap_slots():
    ...
    if tariff_type == "fixed":
        ...
        if n_slots == 0:
            ...
            return []
        if not config.get("has_pv", True):
            result = slots[:min(len(slots), config.get("max_cheap_slots", 8))]
        else:
    ...
    with config_lock:
        has_pv = config.get("has_pv", True)
        max_slots = int(config.get("max_cheap_slots", 8))

    if not has_pv:
        extra = max_slots - len(negative)
        if extra < 0:
            extra = 0
        extra = min(len(normal), extra)
    else:
...
def set_mode_protect(reason: str):
    ...
    with ems_lock:
        ems_state["output_priority"] = OUTPUT_SUB
...
def set_mode_cheap(reason: str, price_now: float = None):
    """Olcsó slot: töltünk – ha price <=0 akkor E204=Line/Grid(1) + E20F=Mains(1), különben E204=SUB(3) + E20F=Hybrid(2)."""
    if price_now is not None and price_now <= 0:
        output_mode = 1  # Line/Grid priority
        charge_mode = MODE_MAINS_PREFERRED  # 1: Mains preferred
        mode_desc = "Cheap (free/negative) – Grid Priority + Mains Charging"
    else:
        output_mode = OUTPUT_SUB  # 3
        charge_mode = MODE_HYBRID  # 2
        mode_desc = "Cheap – Grid Charging"
    
    ok = write_register(REG_OUTPUT_PRI, output_mode)
    time.sleep(3)
    write_register(REG_CHARGE_PRI, charge_mode)
    with ems_lock:
        ems_state["output_priority"] = output_mode
        ems_state["current_mode"] = mode_desc
        ems_state["charge_mode"]      = {0: 'PV-preferred', 1: 'Mains-preferred', 2: 'Hybrid', 3: 'PV-only'}.get(charge_mode, str(charge_mode))
        ems_state["last_mode_change"] = datetime.now().strftime("%a %d-%b %H:%M")
        ems_state["last_action"]  = f"{datetime.now()}: E204={output_mode}+E20F={charge_mode} [{reason}] ok={ok}"
...
def set_mode_expensive(reason: str):
    ...
    with ems_lock:
        if ems_state["current_mode"] != "Expensive – Solar/Battery":
            ems_state["last_mode_change"] = datetime.now().strftime("%a %d-%b %H:%M")
        ems_state["output_priority"] = OUTPUT_SBU
...
def get_panel_kwp():
    """Get PV panel capacity from config."""
    with config_lock:
        return config.get("panel_kwp", 1.76)

# For backward compatibility, set PANEL_KWP after config load
PANEL_KWP = get_panel_kwp()
...
def update_optimizer_learning(records: list):
    ...
    # Only consider days with PV generation > 0.1 kWh to avoid skewing by disconnected panels
    pv_days = [r for r in recent if r["pv_kwh"] > 0.1]
    if pv_days:
        avg_pv = sum(r["pv_kwh"] for r in pv_days) / len(pv_days)
        opt["avg_daily_pv_kwh"] = round(avg_pv, 2)
        print(f"[OPT] PV avg updated from {len(pv_days)} days with PV > 0.1 kWh")
    else:
        # If no PV days, set to 0 if no panels configured, else keep previous
        with config_lock:
            if not config.get("has_pv", True):
                opt["avg_daily_pv_kwh"] = 0.0
                print("[OPT] No PV panels and no PV days — setting avg_daily_pv_kwh to 0")
            else:
                print(f"[OPT] No days with PV > 0.1 kWh in recent data, keeping previous avg_daily_pv_kwh")
...
def compute_dynamic_target_soc() -> tuple:
    ...
    if not wd_patterns:
        if has_pv:
            ...
            needed_kwh = overnight_kwh * 1.1
            reason = f"overnight-estimate({overnight_kwh:.1f}kWh→"
        else:
            # No PV and no patterns: plan for the full day from avg consumption
            needed_kwh = avg_cons_kwh * 1.1
            reason = f"full-day-estimate({avg_cons_kwh:.1f}kWh→"
        target_soc = (needed_kwh / BATTERY_KWH) * 100
        target_soc = max(float(min_soc), min(float(hard_max), target_soc))
        return round(target_soc, 1), reason + f"{target_soc:.0f}%)"
    ...
    steps        = 0
    max_steps    = 24 if not has_pv else 24
    ...
    while steps < max_steps:
        ...
        steps   += 1
        if has_pv and current.date() > now_uk.date() and current.hour >= solar_start:
            break

    needed_kwh = total_kwh * 1.10   # +10% margin
    target_soc = (needed_kwh / BATTERY_KWH) * 100
    if not has_pv:
        # No PV: plan for the next 24h and allow higher SOC if cheap charging is available.
        target_soc = max(float(min_soc), min(float(hard_max), target_soc))
    else:
        # With PV, be conservative and let solar cover the daytime surplus.
...
def compute_required_slots() -> int:
    ...
    # If no PV panels configured, force PV to 0 and ignore learning
    with config_lock:
        if not config.get("has_pv", True):
            avg_pv = 0.0
            pv_scale = 0.0
            print("[OPT] No PV panels configured — forcing PV generation to 0, ignoring past data")
            ...
            if not hc_active and HIGH_CONSUMER_ENTER:
                ...
                if in_cheap:
                    ...
                    if chg_state_live == 8:
                        ...
                        set_mode_cheap(cheap_reason, price_now)
                    with ems_lock:
                        ems_state["last_error"] = None
  ...
  <div class="card" style="margin-bottom:12px;">
    ...
    <div id="manual-actions" class="manual-actions disabled">
      ...
      <div style="display:flex;gap:8px;margin-bottom:10px;">
        ...
        <button class="btn btn-blue" style="flex:1;" onclick="manualMode('grid_priority')">🏠⚡ Grid Priority<br><small style="opacity:.7">Line/Grid · grid priority</small></button>
      </div>
      ...
      <div style="display:flex;gap:8px;">
        ...
        <button class="btn btn-red" style="flex:1;" onclick="setChargeMode(1)">🔌 Mains Preferred<br><small style="opacity:.7">Grid charges</small></button>
      </div>
    </div>
  </div>
...
<div class="modal-bg" id="modal-bg" onclick="if(event.target===this)closeSettings()">
  <div class="modal">
    ...
    <div style="border:1px solid var(--border);border-radius:8px;padding:12px;background:var(--bg3);margin-bottom:10px;">
      <div class="field-label" style="margin-bottom:10px;">☀️ Solar PV Configuration</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        <div class="field-group" style="margin-bottom:0;">
          <div class="field-label">Has solar panels?</div>
          <select id="s-has-pv">
        ...
        <div class="field-group" style="margin-bottom:0;">
          <div class="field-label">Panel capacity (kWp)</div>
          <input id="s-panel-kwp" type="number" step="0.01" min="0" placeholder="1.76">
        </div>
      </div>
      <div style="font-size:10px;color:var(--muted);margin-top:6px;">Set to 0 kWp if no panels. Learning adapts to changes.</div>
    </div>
    <div style="border:1px solid var(--border);border-radius:8px;padding:12px;background:var(--bg3);margin-bottom:10px;">
...
async function refreshEMS(){
  ...
  else if(d.current_mode&&d.current_mode.includes('Grid Priority')){mb.textContent='GRID PRIORITY';mb.className='mode-badge cheap';}
  ...
  const outputPriorityLabel = d.output_priority === 1 ? 'Line/Grid' : d.output_priority === 2 ? 'SBU' : d.output_priority === 3 ? 'SUB' : d.output_priority === 0 ? 'Solar Only' : '--';
  row('Output priority', outputPriorityLabel, 'var(--muted)');
  const isCheap=d.current_mode&&d.current_mode.includes('Cheap');
  const isProtect=d.current_mode&&d.current_mode.includes('Protect');
  const isGridPriority=d.current_mode&&d.current_mode.includes('Grid Priority');
  const isManual=d.control_mode==='manual' || (d.current_mode&&d.current_mode.includes('Manual'));
  let modeLabel;
  if(isManual){
    modeLabel = isGridPriority ? '✋🏠⚡ Manual Grid Priority' : isCheap ? '✋🔋⚡ Manual Grid Charging' : isProtect ? '✋⚠️ Manual Protect' : '✋ MANUAL';
  } else {
    modeLabel = isGridPriority ? '🏠⚡ Grid Priority' : isCheap ? '🔋⚡ Grid Charging' : isProtect ? '⚠️ Battery Protect (Grid Load)' : '☀️🔋 Solar/Battery';
  }
  const modeCol=isCheap||isGridPriority?'var(--green)':isProtect?'var(--red)':'var(--amber)';
...
function setControlMode(mode){
  ...
  fetch('/api/set_control_mode',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode})})
    .then(r=>r.json()).then(d=>{showToast(d.message||mode,d.success);if(d.success)refreshEMS();});
}
...
async function openSettings(){
  ...
  s('s-has-pv').value=c.has_pv!==false?'true':'false';
  s('s-panel-kwp').value=c.panel_kwp||1.76;
...
async function saveSettings(){
  ...
  const payload={
    ...
    has_pv: v('s-has-pv')==='true',
    panel_kwp: parseFloat(v('s-panel-kwp')||'1.76'),
...
def api_telemetry():
    ...
    if t.get("output_priority") is None:
        with ems_lock:
            t["output_priority"] = ems_state.get("output_priority")
...
def api_settings():
    ...
    with config_lock:
        ...
        config["has_pv"]              = bool(data.get("has_pv", config.get("has_pv", True)))
        config["panel_kwp"]           = float(data.get("panel_kwp", config.get("panel_kwp", 1.76)))
    ...
    # Force re-compute optimizer if PV config changed
    with config_lock:
        if "has_pv" in data or "panel_kwp" in data:
            print("[SETTINGS] PV config changed, updating optimizer immediately...")
            # Force reload config to ensure PANEL_KWP is updated
            load_config()
            # Re-compute optimizer with new PV settings
            opt = load_optimizer()
            # If no PV, set avg_pv to 0 immediately
            if not config.get("has_pv", True):
                opt["avg_daily_pv_kwh"] = 0.0
                opt["pv_scale_factor"] = 0.0
                save_optimizer(opt)
                print("[SETTINGS] No PV configured — set avg_daily_pv_kwh to 0 in optimizer")
            # Re-run slot computation with updated config
            cheap_slots = compute_cheap_slots()
            with ems_lock:
                ems_state["cheap_slots"] = [
                    {"start": c["start"].astimezone(UK_TZ).isoformat(),
...
def api_get_config():
    ...
    return jsonify({
        ...
        "has_pv":               c.get("has_pv", True),
        "panel_kwp":            c.get("panel_kwp", 1.76),
...
def api_set_mode_manual():
    ...
    try:
        ...
        elif mode == "grid_priority":
            ok = write_register(REG_OUTPUT_PRI, 1)
            time.sleep(3)
            write_register(REG_CHARGE_PRI, MODE_MAINS_PREFERRED)
            with ems_lock:
                ems_state["output_priority"] = 1
                ems_state["current_mode"] = "Manual – Grid Priority + Mains Charging"
                ems_state["charge_mode"] = "Mains-preferred"
                ems_state["last_mode_change"] = datetime.now().strftime("%a %d-%b %H:%M")
                ems_state["last_action"] = f"{datetime.now()}: E204=1+E20F=1 [manual grid priority] ok={ok}"
            return jsonify({"success": True, "message": "✅ E204=Line/Grid(1) + E20F=Mains(1) – Grid priority + mains charging"})
        else:
...
def api_set_charge_mode():
    ...
    if ctrl != "manual":
        return jsonify({"success": False, "message": "❌ Switch to MANUAL first!"})
    try:
        if mode == 1:  # Mains preferred - also set output to Line/Grid priority
            ok = write_register(REG_OUTPUT_PRI, 1)
            time.sleep(3)
            with ems_lock:
                ems_state["output_priority"] = 1
        ...
        return jsonify({"success": True, "message": f"✅ E204={1 if mode==1 else 'unchanged'} + E20F={names[mode]} set"})
    except Exception as e:
        return jsonify({"success": False, "message": str(e)})
